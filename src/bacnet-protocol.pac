...
type Forwarded_NPDU(is_orig: bool) = record {
    bacnet_ip       : uint32;
    bacnet_port     : uint16;
    npdu            : NPDU_Header(is_orig, 0x04, bacnet_ip, bacnet_port);
};
...
type Distribute_Broadcast_to_Network(is_orig: bool) = record {
    npdu             : NPDU_Header(is_orig, 0x09, 0, 0);
}
...
type Original_Unicast_NPDU(is_orig: bool) = record {
    npdu             : NPDU_Header(is_orig, 0x0A, 0, 0);
}
...
type Original_Broadcast_NPDU(is_orig: bool) = record {
    npdu             : NPDU_Header(is_orig, 0x0B, 0, 0);
}
...
## ------------------------------------------------------------------------------------------------
type NPDU_Header(is_orig: bool, bvlc_function: uint8, forwarded_bacnet_ip: uint32, forwarded_bacnet_port: uint16) = record {
    ...
    overview: bool = $context.flow.process_bacnet_npdu_header(is_orig, bvlc_function, forwarded_bacnet_ip, forwarded_bacnet_port, has_npdu_message ? npdu_message_exists : 0, has_destination ? destination_exists : 0, has_source ? source_exists : 0, has_hop_count, has_hop_count ? hop_count_value : 0xFF);
};
